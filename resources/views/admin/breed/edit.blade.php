            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Chỉnh sửa giống chó/mèo</h1>
                    </div>

                    <div class="card shadow mb-4">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">Chỉnh sửa giống</h6>
                        </div>
                        <div class="card-body">
                            ...
                            <form action="{{ route('admin.breeds.update', $breed) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')

                                <div class="mb-3">
                                    <label for="name" class="form-label">Tên giống</label>
                                    <input type="text" class="form-control" id="name" name="name" value="{{ old('name', $breed->name) }}" required>
                                </div>

                                <div class="mb-3">
                                    <label for="type" class="form-label">Loại</label>
                                    <select class="form-select" id="type" name="type" required>
                                        <option value="dog" {{ old('type', $breed->type) == 'dog' ? 'selected' : '' }}>Chó</option>
                                        <option value="cat" {{ old('type', $breed->type) == 'cat' ? 'selected' : '' }}>Mèo</option>
                                    </select>
                                </div>

                                <div class="mb-3">
                                    <label for="description" class="form-label">Thông tin giống</label>
                                    <textarea class="form-control" id="description" name="description" rows="4">{{ old('description', $breed->description) }}</textarea>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    @if ($breed->image)
                                        <div class="mb-2">
                                            <img src="{{ asset('storage/' . $breed->image) }}" alt="{{ $breed->name }}" style="max-width: 150px; height: auto;">
                                        </div>
                                    @endif
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <button type="submit" class="btn btn-primary">Cập nhật giống</button>
                                </div>
                            </form>
                        </div>
                    </div>
